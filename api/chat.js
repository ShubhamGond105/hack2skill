...
export default async function handler(req, res) {
  ...
  if (!process.env.OPENROUTER_API_KEY) {
    ...
    return res.status(500).json({ error: 'API key not set.', code: 'MISSING_API_KEY' });
  }

  const clientIP = req.headers['x-forwarded-for'] || req.socket?.remoteAddress || 'unknown';
  if (isRateLimited(clientIP)) {
    return res.status(429).json({ error: 'Too many requests. Wait a minute.', code: 'RATE_LIMITED' });
  }
  ...
  if (lastMsg && lastMsg.content && lastMsg.content.length > 1000) {
    return res.status(400).json({ error: 'Message too long (max 1000 chars)', code: 'MESSAGE_TOO_LONG' });
  }
  ...
  try {
    const apiRes = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'HTTP-Referer': 'https://bharat-chunav-sahayak.vercel.app',
    ...
    if (!apiRes.ok) {
      ...
      if (apiRes.status === 429) return res.status(429).json({ error: 'Rate limit reached. Try shortly.', code: 'AI_RATE_LIMITED' });
